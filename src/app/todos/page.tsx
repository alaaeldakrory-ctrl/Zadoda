...
export default function TodosPage() {
  ...
  const handleCreateMemo = (personId: string) => {
    setEditingMemo({ personId, title: '', content: '' });
  ...
  const handleSaveMemo = () => {
    if (!editingMemo || !editingMemo.personId || !editingMemo.title?.trim()) return;
    ...
    <AppLayout>
      <div className="max-w-[1600px] mx-auto space-y-8">
        <div className="flex flex-col gap-1">
          <h1 className="text-4xl font-black tracking-tight">{t.todos}</h1>
          <p className="text-base text-muted-foreground font-bold opacity-70">Your personal list of reminders and tasks.</p>
        </div>
                ...
                <div className="mt-3 flex gap-2 justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                  {!memo.completed && (
                    <Button
                      ...
                      onClick={(e) => { e.stopPropagation(); handleScheduleToCalendar(memo); }}
                      title={t.addEvent}
                    >
                  ...
                  <Button
                    ...
                    onClick={(e) => { e.stopPropagation(); handleEditMemo(memo); }}
                  >
                ...
                <div className="flex-1 space-y-2">
                  {personMemos.length === 0 ? (
                    <div className="h-48 rounded-[2rem] border-2 border-dashed flex flex-col items-center justify-center p-6 text-center space-y-3 opacity-30 grayscale">
                      ...
                      <p className="text-xs font-bold uppercase tracking-widest">Nothing here yet</p>
                    </div>
                  ) : (
                    <>
        ...
        <DialogContent className="rounded-3xl sm:max-w-[500px] border-2">
          <DialogHeader>
            <DialogTitle className="text-2xl font-black">
              {editingMemo?.id ? t.edit : t.addTodo}
            </DialogTitle>
          </DialogHeader>
